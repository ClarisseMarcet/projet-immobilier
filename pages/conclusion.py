...
/* === SUPPRESSION DES 2 BARRES BLANCHES DES TABS STREAMLIT === */
...
def main():
    ...
    st.markdown(
        ...
        <ul>
            <li>Lecture immobilière : territoire <b>{highlight("moins cher" if prix_status=="moins" else ("plus cher" if prix_status=="plus" else "dans la moyenne"), prix_color)}</b> par rapport à la moyenne nationale.</li>
            <li>Lecture climatique : territoire <b>{highlight("moins exposé" if risque_status=="moins" else ("plus exposé" if risque_status=="plus" else "dans la moyenne"), risque_color)}</b> par rapport à la France.</li>
        </ul>
        """,
        unsafe_allow_html=True
    )
